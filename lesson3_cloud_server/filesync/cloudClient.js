...
function postFileToCloud(filePath, username, password, cb) {
  return fs.statAsync(filePath)
    .then(stats => {
      console.log('Trying to sync file', filePath, 'with size', stats.size, ', date', stats.mtime);
      ...
      var uploadUrl = generateUploadUrl(filePath, stats.mtime);
      return agent
        .post(uploadUrl)
...
function generateUploadUrl(filePath, fileDate) {
  var filePathQuery = encodeURI(path.resolve(filePath));
  console.log('filePathQuery generated', filePathQuery);
  return CLOUD_URL + '?filePath=' + filePathQuery + '&fileDate='+fileDate;
}
